...
`;function J(t={}){I("on-call-popup-styles",K);let e=document.getElementById("on-call-template");e||(e=document.createElement("template"),e.id="on-call-template",e.innerHTML=`
        ...
        `,document.body.appendChild(e)),document.querySelector(".on-call-popup-overlay")||document.body.appendChild(e.content.cloneNode(!0));const[n,o]=(t.onCallTime||"17:59").split(":"),a=Number(n)||17,l=Number(o)||59,i=document.querySelector(".on-call-popup-overlay"),r=document.querySelector(".on-call-popup"),c=document.querySelector(".on-call-btn-urgent"),s=document.querySelector(".on-call-btn-normal");if(!i||!r||!c||!s){console.error("[On Call Script] Elementos do popup não foram encontrados.");return}function u(){const d=new Date,y=d.getHours(),g=d.getMinutes();return y>a||y===a&&g>=l}let p=null;function b(){if(!p)return;const d=p.getBoundingClientRect();r.style.display="block";const y=r.offsetWidth,g=r.offsetHeight;let x=0,h=0,w=r.parentElement;for(;w&&w!==document.documentElement&&w!==document.body;){const k=window.getComputedStyle(w);if(k.transform!=="none"||k.perspective!=="none"||k.filter!=="none"||k.willChange==="transform"){const j=w.getBoundingClientRect();x=j.left,h=j.top;break}w=w.parentElement}let q=d.top-h-g-10,L=d.right-x-y;q<10&&(q=d.bottom-h+10),L<10&&(L=10),r.style.top=`${q}px`,r.style.left=`${L}px`}let f=!1;function m(){r.classList.remove("show"),p=null,setTimeout(()=>{i.style.display="none",r.style.display="none"},200)}i.addEventListener("click",m),c.addEventListener("click",function(){const d=p;m(),d&&(f=!0,d.click(),f=!1)}),s.addEventListener("click",function(){const d=p;m(),d&&(f=!0,d.click(),f=!1)}),document.addEventListener("click",function(d){if(f)return;const y=d.target.closest(".place-request-footer .place-request-btn");if(y){if(!/\/catalog\/items\/\d+/.test(window.location.pathname))return;u()&&(d.preventDefault(),d.stopPropagation(),p=y,b(),i.style.display="block",r.offsetWidth,r.classList.add("show"))}},!0),window.addEventListener("resize",b)}function Y(){let t=document.currentScript;if(t&&!t.hasAttribute("data-enable-grid")&&!t.hasAttribute("data-enable-on-call")&&(t=null),!t){const l=Array.from(document.querySelectorAll('script[data-enable-grid], script[data-enable-on-call], script[src*="freshservice-target-cdn"], script[src*="bundle.js"]'));t=l.find(i=>i.hasAttribute("data-enable-grid")||i.hasAttribute("data-enable-on-call"))||l[0]}const e="[Custom Freshservice Scripts @ Target.it]";t?Object.keys(t.dataset).length>0||console.warn(`${e} [DEBUG] Tag <script> localizada (src="${t.src}"), porém não possui atributos "data-*" configurados.`):console.warn(`${e} [DEBUG] Nenhuma tag <script> correspondente foi localizada no DOM.`);const{enableGrid:n,enableOnCall:o,onCallTime:a="17:59"}=t?.dataset??{};return{isGridEnabled:n==="true",isOnCallEnabled:o==="true",onCallTime:a}}function T(){const e="[Custom Freshservice Scripts @ Target.it]";console.log(`${e} Versão: v26`);const n=Y(),o=[{key:"grid",name:"Grid System",init:W,enabled:n.isGridEnabled,config:{}},{key:"on-call",name:"On-Call Intercept Popup",init:J,enabled:n.isOnCallEnabled,config:{onCallTime:n.onCallTime}}],a=o.filter(i=>i.enabled),l=o.filter(i=>!i.enabled);if(a.length===0&&console.warn(`${e} Nenhum script foi ativado.`),l.length>0){console.log(`${e} Scripts disponíveis mas inativos (para ativar, adicione os atributos correspondentes na tag <script> do CDN):`);const i=l.map(r=>({Script:r.name,"Chave do Atributo":`data-enable-${r.key}="true"`,"Configurações Disponíveis":Object.keys(r.config||{}).map(c=>`data-${c}="..."`).join(", ")||"Nenhuma"}));console.table(i)}a.length!==0&&a.forEach(i=>{try{i.init(i.config),console.log(`${e} ${i.name} inicializado com sucesso.`)}catch(r){console.error(`${e} Erro ao inicializar ${i.name}:`,r)}})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",T):T()})();
